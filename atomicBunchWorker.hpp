...
namespace Fang::Test
{
    ...
    void doWork3(work3 & w)
    {
        long long nowWorks;
        std::multimap<unsigned long long,unsigned long long> resultLocal;
        long long initial =((long long)(w.tasks.size()))*0.7/threadnum;
        nowWorks=w.process.fetch_sub((long long)(initial));
        for(long long i=nowWorks;i>nowWorks-initial && i>=0;--i)
        {
            resultLocal.emplace(i,Fang::Test::doSomething(w.tasks[i]));
        }
    ...
    auto work3Start(work3 & w)
    {
        auto now = std::chrono::high_resolution_clock::now();
        for(unsigned int i = 0;i<threadnum;++i)
        {
            w.pool.emplace_back(doWork3,std::ref(w));
        }
        ...
        std::println("atomic bunch 组工作完成，用时：{}",std::chrono::duration<double,std::milli>(endNow-now));
        unsigned int k = 1;
